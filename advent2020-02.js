...
/*****************
 *  MISSION: use the pattern to see if password matches.  Password should contain a number of the given letter in the given range
 * STRATEGY: Count the number of times the letter appears.  see if it's in the given range.  Count is easiest with regex. Add to 
 *           valid array if so, and return the count at the end.
 */
...
function checkSubArray(strNumbers) {
    ...
    strNumbers.forEach(element => {
        ...
        let pwd = element.split(":")[1];
        let min = rule.split("-")[0];
        let max = rule.split("-")[1].split(" ")[0];
        ...
        {
            console.log(`valid: ${pwd} contains between ${min} and ${max} instances of ${substr}`);
            valid.push(element);
        }
        else
        {
            console.log(`INVALID: ${pwd} contains between ${min} and ${max} instances of ${substr}`);
